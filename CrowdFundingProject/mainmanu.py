...
userid = 0
def screan():
    print("hello dear, are you fine. the is program for Crowd Funding ")
    isuser = input("Are you have a user in it? if yes pres (y) or no (n) : ")
    while True:
        if isuser == 'y':
            userid = login()
            break
            ...
            if isuser == 's':
                break
    if userid > 0:
        operat = input("if you wont create a new project pres(c), \nEdit your project pres(e), "
